...
async function getUsersByUE(req, res) {
    const ueCode = req.params.code;

    try {
        const db = getDB();
        const users = await db.collection("users").find({
            registeredUEs: { $elemMatch: { code: ueCode } }
        }).toArray();

        const usersWithoutPasswords = users.map(({ password, ...u }) => u);
        res.json(usersWithoutPasswords);
    } catch (err) {
        console.error("Erreur récupération utilisateurs UE :", err);
...
module.exports = { getAllUsers, getSpecificUser, createUser, updateUser, deleteUser, getUsersByUE };